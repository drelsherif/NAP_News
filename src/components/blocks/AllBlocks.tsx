...
export function HeaderBlock({ block, theme, editable, onUpdateBlock }: { block: HeaderBlockType; theme: T; editable?: boolean; newsletter: any; onUpdateBlock: (c: Partial<HeaderBlockType>) => void }) {
  ...
  return (
    <div style={{ background: bg, padding: '48px 40px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
      ...
      <h1 style={{ fontFamily: theme.fontDisplay, fontSize: 46, fontWeight: 400, color: '#fff', margin: '0 0 10px', letterSpacing: '-0.02em', lineHeight: 1.1 }}>
        {editable ? (
          <EditableText value={block.title} placeholder="Newsletter title" onCommit={(v) => onUpdateBlock({ title: v })} style={{ display: 'inline-block', minWidth: 240 }} />
        ) : (
          block.title
        )}
      </h1>
      <p style={{ fontFamily: theme.fontDisplay, fontSize: 20, fontStyle: 'italic', color: 'rgba(255,255,255,0.85)', margin: '0 0 18px', lineHeight: 1.3 }}>
        {editable ? (
          <EditableText value={block.subtitle} placeholder="Subtitle" onCommit={(v) => onUpdateBlock({ subtitle: v })} style={{ display: 'inline-block', minWidth: 240 }} />
        ) : (
          block.subtitle
        )}
      </p>
      ...
      <p style={{ fontFamily: theme.fontBody, fontSize: 13, color: 'rgba(255,255,255,0.65)', margin: 0, letterSpacing: '0.02em' }}>
        {editable ? (
          <EditableText value={block.tagline} placeholder="Tagline" onCommit={(v) => onUpdateBlock({ tagline: v })} style={{ display: 'inline-block', minWidth: 240 }} />
        ) : (
          block.tagline
        )}
      </p>
...
export function TickerBlock({ block, theme }: { block: TickerBlockType; theme: T; newsletter: any; onUpdateBlock: any }) {
  ...
  return (
    <div style={{ background: block.backgroundColor || theme.primary, overflow: 'hidden', height: 40, display: 'flex', alignItems: 'center' }}>
...
export function SectionDividerBlock({ block, theme, editable, onUpdateBlock }: { block: SectionDividerBlockType; theme: T; editable?: boolean; newsletter: any; onUpdateBlock: (c: Partial<SectionDividerBlockType>) => void }) {
  if (block.style === 'bold') {
    return (
      <div style={{ background: theme.primary, padding: '18px 40px', display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div>
          ...
          <div style={{ fontFamily: theme.fontDisplay, fontSize: 22, color: '#fff', lineHeight: 1.15 }}>
            {editable ? (
              <EditableText value={block.label} placeholder="Section title" onCommit={(v) => onUpdateBlock({ label: v })} style={{ display: 'inline-block', minWidth: 200 }} />
            ) : (
              block.label
            )}
          </div>
    ...
    <div style={{ padding: '32px 40px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: block.description ? 10 : 0 }}>
        <div style={{ fontFamily: theme.fontMono, fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: theme.muted, whiteSpace: 'nowrap' }}>
          ...
          {editable ? (
            <EditableText value={block.label} placeholder="Section title" onCommit={(v) => onUpdateBlock({ label: v })} style={{ display: 'inline-block', minWidth: 200, color: theme.muted }} />
          ) : (
    ...
    <div style={pad(24, 40)}>
      {block.sectionTitle !== undefined && (
        <h2 style={{ fontFamily: theme.fontDisplay, fontSize: 26, color: theme.text, margin: '0 0 20px', fontWeight: 400 }}>
          {editable ? (
            <EditableText value={block.sectionTitle || ''} placeholder="Section title" onCommit={(v) => onUpdateBlock({ sectionTitle: v })} style={{ display: 'inline-block', minWidth: 240 }} />
          ) : (
            block.sectionTitle
          )}
        </h2>
...
function ArticleCard({ article: art, theme, layout, editable, onUpdate }: { article: Article; theme: T; layout: string; editable?: boolean; onUpdate: (c: Partial<Article>) => void }) {
  ...
  return (
    <div style={{ border: `1px solid ${theme.border}`, borderRadius: 12, overflow: 'hidden', background: theme.surface, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: layout === 'compact' ? '10px 14px' : '16px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>
          <span style={{ fontFamily: theme.fontMono, fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: theme.muted }}>
            {editable ? (
              <EditableText value={art.source || ''} placeholder="Source" onCommit={(v) => onUpdate({ source: v })} style={{ display: 'inline-block', minWidth: 120, color: theme.muted }} />
            ) : (
              art.source
            )}
          </span>
        ...
        <h3 style={{ fontFamily: theme.fontDisplay, fontSize: layout === 'compact' ? 16 : 20, color: theme.text, margin: '0 0 10px', lineHeight: 1.25, fontWeight: 400 }}>
          {editable ? (
            <EditableText value={art.title || ''} placeholder="Article title" multiline={false} onCommit={(v) => onUpdate({ title: v })} style={{ display: 'inline-block', minWidth: 180 }} />
          ) : (
  ...
  return (
    <div style={pad(24, 40)}>
      <div style={{ border: `1px solid ${theme.border}`, borderRadius: 14, overflow: 'hidden', background: theme.surface, display: 'flex', flexDirection: hasImage ? (isLeft ? 'row' : 'row-reverse') : 'column' }}>
        ...
        <div style={{ flex: 1, padding: 28, borderLeft: hasImage && isLeft ? `4px solid ${block.accentColor || theme.accent}` : undefined }}>
...
function SpotlightContent({ art, theme, accentColor, editable, onUpdate }: { art: Article; theme: T; accentColor: string; editable?: boolean; onUpdate: (c: Partial<Article>) => void }) {
  return (<>
    <h2 style={{ fontFamily: theme.fontDisplay, fontSize: 28, color: theme.text, margin: '0 0 12px', fontWeight: 400, lineHeight: 1.2 }}>
      {editable ? (
        <EditableText value={art.title || ''} placeholder="Spotlight title" onCommit={(v) => onUpdate({ title: v })} style={{ display: 'inline-block', minWidth: 240 }} />
      ) : (
    ...
    <div style={pad(28, 40)}>
      <div style={{ marginBottom: 20 }}>
        ...
        <h2 style={{ fontFamily: theme.fontDisplay, fontSize: 26, color: theme.text, margin: '0 0 6px', fontWeight: 400 }}>
          {editable ? (
            <EditableText value={block.heading || ''} placeholder="Heading" onCommit={(v) => onUpdateBlock({ heading: v })} style={{ display: 'inline-block', minWidth: 240 }} />
          ) : (
            block.heading
          )}
        </h2>
        ...
        {block.source !== undefined && (
          <div style={{ fontFamily: theme.fontMono, fontSize: 11, color: theme.muted, marginTop: 6 }}>
            Source:{' '}
            {editable ? (
              <EditableText value={block.source || ''} placeholder="Source" onCommit={(v) => onUpdateBlock({ source: v })} style={{ display: 'inline-block', minWidth: 140, color: theme.muted }} />
            ) : (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div style={{ border: `1px solid ${theme.border}`, borderRadius: 12, padding: 20, background: theme.background }}>
          <div style={{ fontFamily: theme.fontMono, fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#C0392B', fontWeight: 700, marginBottom: 10 }}>
            {editable ? <EditableText value={block.leftTitle || ''} placeholder="Left title" onCommit={(v) => onUpdateBlock({ leftTitle: v })} style={{ display: 'inline-block', minWidth: 140, color: '#C0392B' }} /> : block.leftTitle}
          </div>
        ...
        <div style={{ border: `1px solid ${theme.accent}44`, borderRadius: 12, padding: 20, background: theme.accent + '0A' }}>
          <div style={{ fontFamily: theme.fontMono, fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: theme.accent, fontWeight: 700, marginBottom: 10 }}>
            {editable ? <EditableText value={block.rightTitle || ''} placeholder="Right title" onCommit={(v) => onUpdateBlock({ rightTitle: v })} style={{ display: 'inline-block', minWidth: 140, color: theme.accent }} /> : block.rightTitle}
          </div>
          <p style={{ fontFamily: theme.fontBody, fontSize: 14, color: theme.text, margin: 0, lineHeight: 1.65 }}>
  ...
  return (
    <div style={{ padding: '16px 40px' }}>
      <div style={{ fontFamily: theme.fontMono, fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: theme.muted, marginBottom: 8 }}>
        💻{' '}
        {editable ? (
          <EditableText value={block.label || ''} placeholder="Embed label" onCommit={(v) => onUpdateBlock({ label: v })} style={{ display: 'inline-block', minWidth: 140, color: theme.muted }} />
        ) : (
          block.label
        )}
      </div>
...
export function PromptMasterclassBlock({ block, theme, editable, onUpdateBlock }: { block: PromptMasterclassBlockType; theme: T; editable?: boolean; newsletter: any; onUpdateBlock: (c: Partial<PromptMasterclassBlockType>) => void }) {
  return (
    <div style={pad(28, 40)}>
      ...
      <h2 style={{ fontFamily: theme.fontDisplay, fontSize: 26, color: theme.text, margin: '0 0 6px', fontWeight: 400 }}>
        {editable ? <EditableText value={block.heading || ''} placeholder="Heading" onCommit={(v) => onUpdateBlock({ heading: v })} style={{ display: 'inline-block', minWidth: 240 }} /> : block.heading}
      </h2>
      <div style={{ fontFamily: theme.fontBody, fontSize: 14, color: theme.muted, marginBottom: 20 }}>
        {editable ? (
          <>
            <EditableText value={block.step || ''} placeholder="Step" onCommit={(v) => onUpdateBlock({ step: v })} style={{ display: 'inline-block', minWidth: 160, color: theme.muted }} />
            {' · Framework: '}
            <strong><EditableText value={block.framework || ''} placeholder="Framework" onCommit={(v) => onUpdateBlock({ framework: v })} style={{ display: 'inline-block', minWidth: 180 }} /></strong>
          </>
        ) : (
          <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 20 }}>
        {(block.steps || []).map((step, i) => (
          <div key={step.letter} style={{ border: `1px solid ${theme.border}`, borderRadius: 10, padding: 14, background: theme.surface }}>
            <div style={{ fontFamily: theme.fontDisplay, fontSize: 38, color: colors[i] || theme.primary, lineHeight: 1, marginBottom: 8, fontWeight: 400 }}>{step.letter}</div>
            <div style={{ fontFamily: theme.fontMono, fontSize: 9, letterSpacing: '0.14em', textTransform: 'uppercase', color: theme.muted, marginBottom: 8 }}>
              {editable ? (
                <EditableText value={step.name || ''} placeholder="Step name" onCommit={(v) => updateStep(i, { name: v })} style={{ display: 'inline-block', minWidth: 90, color: theme.muted }} />
              ) : (
                step.name
              )}
            </div>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                      <span style={{ fontFamily: theme.fontMono, fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', color: catColor, padding: '2px 7px', background: catColor + '18', borderRadius: 999 }}>{tpl.category}</span>
                    </div>
  ...
  return (
    <div style={pad(28, 40)}>
      ...
      <h2 style={{ fontFamily: theme.fontDisplay, fontSize: 34, color: theme.text, margin: '0 0 20px', fontWeight: 400 }}>
        {editable ? <EditableText value={block.term || ''} placeholder="Term" onCommit={(v) => onUpdateBlock({ term: v })} style={{ display: 'inline-block', minWidth: 200 }} /> : block.term}
      </h2>
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <div style={{ background: theme.primary, borderRadius: 10, padding: '14px 18px', textAlign: 'center' }}>
            <div style={{ fontFamily: theme.fontDisplay, fontSize: 28, color: '#fff', lineHeight: 1 }}>
              {editable ? <EditableText value={block.year || ''} placeholder="Year" onCommit={(v) => onUpdateBlock({ year: v })} style={{ display: 'inline-block', minWidth: 60, color: '#fff' }} /> : block.year}
            </div>
        ...
        <div style={{ flex: 1 }}>
          <h3 style={{ fontFamily: theme.fontDisplay, fontSize: 22, color: theme.text, margin: '0 0 10px', fontWeight: 400 }}>
            {editable ? <EditableText value={block.title || ''} placeholder="Title" onCommit={(v) => onUpdateBlock({ title: v })} style={{ display: 'inline-block', minWidth: 240 }} /> : block.title}
          </h3>
  ...
  return (
    <div style={pad(24, 40)}>
      <h2 style={{ fontFamily: theme.fontDisplay, fontSize: 26, color: theme.text, margin: '0 0 16px', fontWeight: 400 }}>
        ⚡{' '}
        {editable ? <EditableText value={block.heading || ''} placeholder="Heading" onCommit={(v) => onUpdateBlock({ heading: v })} style={{ display: 'inline-block', minWidth: 220 }} /> : block.heading}
      </h2>
          ...
          <div key={hit.id} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', padding: '12px 14px', border: `1px solid ${theme.border}`, borderRadius: 10, background: theme.surface }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: theme.fontBody, fontSize: 14, fontWeight: 600, color: theme.text, marginBottom: 4, lineHeight: 1.3 }}>
                {editable ? (
                  <EditableText value={hit.title || ''} placeholder="Title" onCommit={(v) => updateHit(hit.id, { title: v })} style={{ display: 'inline-block', minWidth: 220 }} />
                ) : (
              ...
              <div style={{ fontFamily: theme.fontMono, fontSize: 10, color: theme.muted, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>
                {editable ? <EditableText value={hit.source || ''} placeholder="Source" onCommit={(v) => updateHit(hit.id, { source: v })} style={{ display: 'inline-block', minWidth: 120, color: theme.muted }} /> : hit.source}
              </div>
  ...
  return (
    <div style={pad(28, 40)}>
      <div style={{ background: `linear-gradient(135deg, ${theme.background}, ${theme.surface})`, border: `1px solid ${theme.border}`, borderRadius: 16, overflow: 'hidden' }}>
        {imgSrc && <div style={{ height: 200, overflow: 'hidden' }}><img src={imgSrc} alt="Humor block" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></div>}
        <div style={{ padding: '32px 36px', textAlign: 'center' }}>
          ...
          <h3 style={{ fontFamily: theme.fontDisplay, fontSize: 22, color: theme.text, margin: '0 0 16px', fontWeight: 400 }}>
            {editable ? <EditableText value={block.heading || ''} placeholder="Heading" onCommit={(v) => onUpdateBlock({ heading: v })} style={{ display: 'inline-block', minWidth: 200 }} /> : block.heading}
          </h3>
          ...
          <p style={{ fontFamily: theme.fontMono, fontSize: 12, color: theme.muted, margin: '0 0 8px' }}>
            {editable ? <EditableText value={block.attribution || ''} placeholder="Attribution" onCommit={(v) => onUpdateBlock({ attribution: v })} style={{ display: 'inline-block', minWidth: 160, color: theme.muted }} /> : block.attribution}
          </p>
...
export function AiSafetyBlock({ block, theme, editable, onUpdateBlock }: { block: AiSafetyBlockType; theme: T; editable?: boolean; newsletter: any; onUpdateBlock: (c: Partial<AiSafetyBlockType>) => void }) {
  const updateUpdate = (id: string, changes: Partial<AISafetyUpdate>) => {
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                    <span style={{ fontFamily: theme.fontMono, fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', color: sev.color, padding: '2px 7px', background: sev.color + '18', borderRadius: 999 }}>
                      {editable ? <EditableText value={update.category || ''} placeholder="Category" onCommit={(v) => updateUpdate(update.id, { category: v })} style={{ display: 'inline-block', minWidth: 90, color: sev.color }} /> : update.category}
                    </span>
                    <span style={{ fontFamily: theme.fontMono, fontSize: 10, color: theme.muted }}>
                      {editable ? <EditableText value={update.date || ''} placeholder="Date" onCommit={(v) => updateUpdate(update.id, { date: v })} style={{ display: 'inline-block', minWidth: 90, color: theme.muted }} /> : update.date}
                    </span>
                  </div>
                  <div style={{ fontFamily: theme.fontBody, fontSize: 14, fontWeight: 600, color: theme.text, marginBottom: 4 }}>
                    {editable ? (
                      <EditableText value={update.title || ''} placeholder="Title" onCommit={(v) => updateUpdate(update.id, { title: v })} style={{ display: 'inline-block', minWidth: 240 }} />
                    ) : (
        ...
        {(block.items || []).slice(0, block.maxItems || 6).map((item) => (
          <div key={item.id} style={{ border: `1px solid ${theme.border}`, borderRadius: 10, overflow: 'hidden', background: theme.surface, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: 14, flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ fontFamily: theme.fontBody, fontSize: 13, fontWeight: 600, color: theme.text, marginBottom: 6, lineHeight: 1.35, flex: 1 }}>
...
export function FooterBlock({ block, theme }: { block: FooterBlockType; theme: T; newsletter: any; onUpdateBlock: any }) {
  return (
    <div style={{ background: theme.primary, padding: '44px 40px 36px', textAlign: 'center' }}>
      {(block.nextIssueDate || block.nextIssueTeaser) && (
        <div style={{ background: 'rgba(255,255,255,0.08)', borderRadius: 10, padding: '14px 20px', marginBottom: 28, display: 'inline-block' }}>
          <div style={{ fontFamily: theme.fontMono, fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', marginBottom: 4 }}>Next Issue</div>
          <div style={{ fontFamily: theme.fontBody, fontSize: 14, color: 'rgba(255,255,255,0.85)' }}>{block.nextIssueDate}{block.nextIssueTeaser ? ` · ${block.nextIssueTeaser}` : ''}</div>
        </div>
      )}
      <div style={{ fontFamily: theme.fontDisplay, fontSize: 22, color: '#fff', marginBottom: 4, fontWeight: 400 }}>{block.institution}</div>
      <div style={{ fontFamily: theme.fontBody, fontSize: 14, color: 'rgba(255,255,255,0.65)', marginBottom: 4 }}>{block.department}</div>
      {block.editors && <div style={{ fontFamily: theme.fontBody, fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 24 }}>{block.editors}</div>}
      <div style={{ display: 'flex', justifyContent: 'center', gap: 24, flexWrap: 'wrap', marginBottom: 24 }}>
        {[block.subscribeUrl && { label: 'Subscribe', url: block.subscribeUrl }, block.websiteUrl && { label: 'Website', url: block.websiteUrl }, block.contactEmail && { label: 'Contact', url: `mailto:${block.contactEmail}` }, block.unsubscribeUrl && { label: 'Unsubscribe', url: block.unsubscribeUrl }].filter(Boolean).map((link: any) => (
          <a key={link.label} href={link.url} target="_blank" rel="noopener noreferrer" style={{ fontFamily: theme.fontBody, fontSize: 12, color: 'rgba(255,255,255,0.6)', textDecoration: 'none', borderBottom: '1px solid rgba(255,255,255,0.2)', paddingBottom: 2 }}>{link.label}</a>
        ))}
      </div>
      <div style={{ height: 1, background: 'rgba(255,255,255,0.1)', maxWidth: 200, margin: '0 auto 20px' }} />
      <p style={{ fontFamily: theme.fontBody, fontSize: 11, color: 'rgba(255,255,255,0.4)', margin: '0 0 8px', maxWidth: 580, marginLeft: 'auto', marginRight: 'auto', lineHeight: 1.6 }}>{block.disclaimer}</p>
      <p style={{ fontFamily: theme.fontMono, fontSize: 10, color: 'rgba(255,255,255,0.3)', margin: 0, letterSpacing: '0.1em' }}>© {block.copyrightYear} {block.institution}</p>
    </div>
  );
}